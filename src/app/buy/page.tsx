...
export default function BuyPage() {
  ...
  const filtered = useMemo(() => {
    return properties.filter((p) => {
      if (p.transactionType !== "buy") return false;
  ...
  return (
    <>
      <section className="pt-32 pb-12 bg-primary">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-3">Buy Property in Dubai</h1>
            <p className="text-gray-400 text-lg max-w-2xl">
              Discover premium freehold properties for sale across Dubai&apos;s most prestigious locations.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-8 bg-warm-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SearchFilters onFilter={setFilters} transactionType="buy" />
        </div>
      </section>
